    ...
    <div class="p-pkg p-pkg-build" v-loading="loading">
        ...
        <div class="m-content">
            ...
            >
                ...
                <!-- 依赖包 -->
                <pkg-module v-if="pkg.is_raw == 0 && done"></pkg-module>
                <!-- 数据内容 -->
                <pkg-items v-if="pkg.is_raw == 0 && done"></pkg-items>
                <!-- 扩展信息 -->
...
import pkg_items from "@/components/dbm/pkg/form/pkg_items.vue";
import pkg_module from "@/components/dbm/pkg/form/pkg_module.vue";
...
export default {
    ...
    computed: {
        ...
        isEditMode() {
            return this.$route.name == "pkg_edit";
        },
        id: function () {
    ...
    methods: {
        ...
        loadPkg() {
            ...
            getMyPkg(this.id)
                .then(async (res) => {
                    ...
                    // 修正数据
                    data.items = Array2String(itemsFields, data.pkg_record?.items || extendPkg.items);
        ...
        async loadModules() {
            const res = await getModuleList(this.id);
            this.$store.commit("SET_PKG", {
                ...this.pkg,
                modules: res.data.data.sort((a, b) => a.priority - b.priority),
            });
            this.$forceUpdate();
        },
