...
export default {
    ...
    methods: {
                ...
                if (valid) {
                    ...
                    publishPkg(data)
                        .then((res) => {
                            ...
                            this.$router.push({ name: "mark_edit", params: { id: res.data.data.id } });
                        })
                        .finally(() => {
                            this.loading = false;
                        });
        ...
        init() {
            this.$store.commit("RESET_PKG", { is_raw: 0, type: 3 });
            getFirstPkg().then((res) => {
                if (!res.data.data) {
                    this.$store.commit("SET_PKG", {
                        ...this.pkg,
                        title: `${User.getInfo().name} 的场景标记数据`,
                        name: "默认版",
                    });
                }
            });
